...
class AssetManager:
    ...
    def create_asset_from_utxo(self, txid, output_index, asset_name, 
metadata):
        # Check if the UTXO exists
        self.cur.execute("SELECT * FROM utxos WHERE txid = ? AND 
output_index = ?", (txid, output_index))
        utxo = self.cur.fetchone()
        
        if not utxo:
            raise ValueError(f"UTXO {txid}:{output_index} does not 
exist.")
        ...
        self.conn.commit()
        return f"Asset '{asset_name}' created from UTXO 
{txid}:{output_index}."
    ...
    def get_assets_by_name(self, asset_name):
        self.cur.execute("SELECT * FROM assets WHERE asset_name = ?", 
(asset_name,))
        return self.cur.fetchall()

    def delete_asset(self, asset_name):
        self.cur.execute("DELETE FROM assets WHERE asset_name = ?", 
(asset_name,))
        self.conn.commit()
...
if __name__ == "__main__":
    ...
    # Example usage
    try:
        print(manager.create_asset_from_utxo(
